...
const calls=[],emails=[];window.location={origin:'https://chococravings.netlify.app',search:''};let view={role:'customer',member:{name:'<img src=x onerror=alert(1)>',phone:'••••••4321'},cards:[{id:'c',number:'CC-P-123',cycle:1,days:9,minimum_spend:0,schedule:[{day:10,item:'Coffee'}]}],rewards:[],stamps:[]};
window.db={auth:{signInWithOtp:async payload=>{emails.push(payload);return {};},getUser:async()=>({data:{user:{id:'u',email:'member@example.test',email_confirmed_at:'2026-09-21'}}})},rpc:async(name,args)=>{calls.push({name,args});return {data:name==='cc_premium_view'?view:{code:'one-time-code',expires_at:new Date(Date.now()+300000).toISOString()}};}};
window.registerAdminTool=()=>{};
vm.runInNewContext(fs.readFileSync('premium-cards.js','utf8'),{window,document:window.document,Intl,Date,console,FormData:class { *[Symbol.iterator](){} }});
...
(async()=>{
 ...
 view={role:'admin',member:null,cards:[],rewards:[],stamps:[],menu:['Coffee'],members:[],reviews:[]};await window.openPremiumCards();assert.ok(window.document.querySelector('[data-form=issue]'));assert.ok(window.document.querySelector('[data-form=email]'));
 window.document.querySelector('[data-form=email]').dispatchEvent(new window.Event('submit',{bubbles:true,cancelable:true}));await tick();await tick();assert.equal(emails.length,1);assert.equal(emails[0].email,'member@example.test');assert.equal(emails[0].options.shouldCreateUser,false);assert.match(emails[0].options.emailRedirectTo,/premium-card=1/);assert.match(window.document.querySelector('.pc-message').textContent,/Verification link requested/);
