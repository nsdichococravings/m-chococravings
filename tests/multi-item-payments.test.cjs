...
const db={from(table){
  ...
  let patch,filters=[];
  const q={select(){return q;},eq(k,v){filters.push([k,v]);return q;},is(k,v){filters.push([k,v]);return q;},single:async()=>({data:copy(row)}),update(v){patch=v;return q;},then(resolve,reject){return Promise.resolve().then(()=>{
    if(fail)return {error:{message:'Offline'}};
    if(casConflict||filters.some(([k,v])=>row[k]!==v))return {data:[]};
...
(async()=>{
 ...
 assert.equal(ctx.icAmount(),30);ctx.icStepQty(2,1);assert.equal(ctx.icAmount(),119);
 ...
 await ctx.icFinalize('upi',null,null);assert.equal(updates,1);assert.equal(row.collected_amount,194);assert.equal(row.status,'collected');assert.equal(JSON.parse(row.items)[0].paidQty,2);assert.equal(JSON.parse(row.items)[2].paidQty,1);
 reset();row.items[2].qty=3;row.total=372;await ctx.openItemCollectPicker('test',0);ctx.icStepQty(2,1);
 assert.equal(ctx.icAmount(),119);await ctx.icFinalize('card',null,null);assert.equal(row.status,'pending');assert.equal(JSON.parse(row.items)[2].paidQty,1);assert.equal(row.collected_amount,194);
 ...
 reset();await ctx.openItemCollectPicker('test',0);row.collected_amount=105;await ctx.icFinalize('upi',null,null);assert.equal(updates,0);assert.match(messages.at(-1),/changed/);
