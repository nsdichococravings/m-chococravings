-- Premium Cards: group visit codes.
-- Lets the member generating a "visit" code (not a reward code) name
-- dining companions by their verified phone number, so staff redeem ONE
-- code and assign the whole group's items in one pass instead of asking
-- every person at the table to individually sign in and show a code.
-- Reward codes stay single-member — rewards are personal.
...
alter table public.cc_premium_codes add column if not exists companion_member_ids uuid[] not null default '{}';
  ...
  if p_payload->>'reward_id' is not null then
   ...
   for v_phone in select value from jsonb_array_elements_text(p_payload->'companion_phones') loop
    select pm.id into v_id from public.cc_premium_members pm join auth.users au on au.id=pm.user_id
     where pm.phone=btrim(v_phone) and not pm.suspended and au.phone_confirmed_at is not null;
