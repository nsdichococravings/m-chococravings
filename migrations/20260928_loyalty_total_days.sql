-- Cumulative total-days tracking across ALL of a customer's numbered
-- cards -- e.g. Card 1's 10-day cycle completed, then 5 stamps so far
-- on Card 2, should show "15 of 100 total days", separate from each
-- card's own day requirement (Card 2 alone still shows "5 / 10").
--
-- completed_days is the running sum of every COMPLETED cycle's own
-- length (the same v_cycle_len complete_cycle already computes from
-- max(schedule day)) -- it does NOT include the customer's current,
-- still-in-progress card. completed_days + the live stamp_count is
-- the lifetime total shown on the card. 100 is the fixed overall
-- journey length the user confirmed earlier (independent of how many
-- days any individual numbered card takes) -- it is a display target
-- only, nothing resets or blocks once a customer passes it.
...
alter table public.cc_loyalty_members add column if not exists completed_days integer not null default 0 check (completed_days>=0);
