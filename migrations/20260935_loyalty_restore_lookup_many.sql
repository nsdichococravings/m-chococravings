-- Restores lookup_many, accidentally dropped when cc_loyalty_command
-- got rewritten in 20260925_loyalty_default_schedule.sql (before the
-- Card Levels work started) and missing from every full-function
-- rewrite since. lookup_many is what the Kitchen ticket uses to batch-
-- fetch loyalty cards for every table order on screen in one round
-- trip (lcLoyaltyMapFor in loyalty-cards.js) -- with it gone, every
-- call failed with "Unknown loyalty action", which lcLoyaltyMapFor's
-- catch-all silently turns into {_unavailable:true}, showing "Lookup
-- unavailable; refresh to retry" on every ticket instead of the real
-- error (refreshing never helped, since the action was actually gone
-- server-side, not a transient failure).
-- Run this ENTIRE file as database owner AFTER 20260934_loyalty_backfill_completed_days.sql.
begin;
do $$ begin
