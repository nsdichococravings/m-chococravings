-- Permanently deletes a customer's loyalty card (and its stamp history)
-- -- for cleaning up a duplicate/stale record like the one that caused
-- the customer's-own-card bug fixed in 20260930 (an old, untouched row
-- for the same phone number left sitting on Card 1 forever). Gated to
-- a super admin only, same as set_cycle_level/apply_level_to_cards --
-- the UI shows the button to any admin, and surfaces the RPC's error
-- if someone without that flag tries it, matching how every other
-- super-admin-only action in this app already works.
--
-- cc_loyalty_stamp_log.member_id has no ON DELETE CASCADE, so this
-- deletes the member's stamp history first, then the member row
-- itself, both inside cc_loyalty_command's own transaction -- either
-- both happen or neither does.
-- Run this ENTIRE file as database owner AFTER 20260929_loyalty_move_card.sql.
begin;
do $$ begin
