-- Replaces the single shared "default schedule" with per-level ones --
-- the physical cards are literally named/numbered Card 1, Card 2, Card
-- 3..., and each one has its own day/reward requirements (e.g. Card 2
-- might be a 10-day card while Card 1 was 15 days). cc_loyalty_members
-- .cycle already tracks which numbered card a customer is currently
-- on; this makes that number mean something schedule-wise instead of
-- just being a counter that reused the same schedule every time.
--
-- New cards start on Card 1 (cycle=1, unchanged) using Card 1's
-- schedule. Completing a cycle looks up the NEW cycle number's own
-- level schedule and switches the card to it; if that level hasn't
-- been defined yet, the card keeps its current schedule rather than
-- being left blank. apply_level_to_cards pushes a level's current
-- schedule onto every EXISTING card sitting on that same cycle number
-- (not every card regardless of level) -- for catching up customers
-- already partway through a card whose schedule just got corrected.
--
-- Supersedes get_default_schedule/set_default_schedule/
-- apply_default_to_all from 20260925_loyalty_default_schedule.sql,
-- which assumed one schedule for every cycle. cc_loyalty_defaults is
-- left in place but unused -- nothing reads it after this migration.
...
create table if not exists public.cc_loyalty_cycle_levels (
 level int primary key check (level >= 1),
 schedule jsonb not null,
 updated_by uuid references auth.users(id),
 updated_at timestamptz not null default now()
);
alter table public.cc_loyalty_cycle_levels enable row level security;
revoke all on public.cc_loyalty_cycle_levels from public,anon,authenticated;
-- No direct grants — every read/write goes through cc_loyalty_command,
-- same as cc_loyalty_members itself.
...
begin
 ...
 if p_action='issue' then
  ...
  select schedule into v_schedule from public.cc_loyalty_cycle_levels where level=1;
  ...
  insert into public.cc_loyalty_members(name,phone,card_code,schedule,stamp_count,issued_by)
   values(btrim(p_payload->>'name'),v_phone,v_code,coalesce(v_schedule,'[]'::jsonb),v_count,auth.uid()) returning * into m;
  if v_count>0 then
