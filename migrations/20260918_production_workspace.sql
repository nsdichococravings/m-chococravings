...
declare
  ...
  v_count integer; v_result jsonb; v_kg numeric; v_labor numeric; v_overhead numeric;
begin
  ...
  if p_action='request' then
    ...
    insert into public.cc_production_movements(batch_id,kind,item_name,location,quantity,unit,unit_cost,actor,command_key) values
      (v_batch.id,'collection_out',v_batch.product_name,'Central kitchen',-v_qty,'pcs',v_batch.unit_cost,v_actor,p_key),
      (v_batch.id,'collection_in',v_batch.product_name,'Main outlet',v_qty,'pcs',v_batch.unit_cost,v_actor,p_key);
    v_id:=v_batch.id;
  end if;
...
begin
  if exists(select 1 from pg_publication where pubname='supabase_realtime') then
    foreach tab in array array['cc_production_requests','cc_production_batches','cc_production_recipes'] loop
