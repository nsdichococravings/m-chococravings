-- "Apply Card N's schedule to customers on Card N" (20260927) only ever
-- touches customers who are ALREADY tagged that cycle number -- it has
-- no way to move a specific customer from Card 1 onto Card 2. Staff hit
-- exactly that gap: created Card 2, hit Apply, and the Card-1 customer
-- they meant to move stayed on Card 1 because the bulk action correctly
-- skips anyone not already on that level.
--
-- set_cycle is the missing per-customer action: directly place ONE
-- customer on a specific numbered card with a given stamp count --
-- for a customer who already has physical progress on a different card
-- than the app currently has them on, or who needs correcting without
-- waiting for them to naturally complete their current cycle. Pulls
-- that level's schedule (falls back to keeping the current one if that
-- level isn't defined yet, same fallback complete_cycle already uses).
-- Does not touch completed_days -- the lifetime total only tracks
-- cycles actually completed through the app, so jumping a customer
-- straight to Card 3 does not retroactively credit Card 1/2's days.
-- Run this ENTIRE file as database owner AFTER 20260928_loyalty_total_days.sql.
begin;
do $$ begin
