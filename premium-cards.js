...
(function () {
 'use strict';
 let dialog, data, currentOrder=null, priorFocus, busy=false, phonePending='', requestSerial=0;
 ...
 function close(){if(busy)return;++requestSerial;dialog.close();dialog.querySelector('.pc-body').innerHTML='';data=null;currentOrder=null;phonePending='';priorFocus?.focus();}
 function init(){
  if(dialog)return;
 ...
 async function open(orderId){
  ...
  try {const user=await window.db.auth.getUser();if(user.error||!user.data.user)throw Error('Please sign in to view your Premium Card.');await refresh();}
 ...
 function memberCard(){
  const m=data.member,c=data.cards?.[0];if(!m||!c)return '<section><h3>Your membership starts here</h3><p>Verify your mobile number, then ask admin to issue your Premium Card.</p><form data-form="phone"><label>Mobile number with country code<input name="phone" type="tel" placeholder="+919876543210" required autocomplete="tel"></label><button class="pc-primary">Send verification SMS</button></form><form data-form="otp"><label>SMS verification code<input name="otp" inputmode="numeric" autocomplete="one-time-code" required></label><button>Verify mobile</button></form><p class="pc-muted">Mobile already verified? Ask admin to issue the card to your account email.</p></section>';
  ...
  return '<section class="pc-membership"><div class="pc-card-top"><span>CHOCOCRAVINGS<br><strong>Premium club</strong></span><span>♛</span></div><h3>'+esc(m.name)+'</h3><p>Verified mobile · '+esc(m.phone)+'</p><div class="pc-card-bottom"><span>'+esc(c.number)+'</span><span>Cycle '+c.cycle+' · '+(m.suspended?'Suspended':days>=100?'Completed':'Active')+'</span></div></section>'
   ...
   +'<p>'+ (stamps.some(s=>s.business_day===today())?'✓ Today’s purchase day is recorded':'Today’s stamp appears after your qualifying order is paid and collected.')+'</p><button class="pc-primary" data-pc="code" '+(m.suspended?'disabled':'')+'>Verify my visit</button><p class="pc-muted">Dining with friends? Show staff your visit code and ask them to assign your items. One friend can pay the combined bill.</p></section>'
 ...
 async function submit(form){if(busy)return;busy=true;const button=form.querySelector('button[type=submit],button:not([type])');if(button)button.disabled=true;status('Saving…');
  try{const fields=Object.fromEntries(new FormData(form)),kind=form.dataset.form;
   if(kind==='phone'){const phone=fields.phone.replace(/\s/g,'');if(!/^\+[1-9]\d{7,14}$/.test(phone))throw Error('Enter country code and mobile number, for example +919876543210.');const res=await window.db.auth.updateUser({phone});if(res.error)throw res.error;phonePending=phone;status('SMS sent. Enter the code below.');return;}
   if(kind==='otp'){if(!phonePending)throw Error('Send a verification SMS from this screen first.');const res=await window.db.auth.verifyOtp({phone:phonePending,token:fields.otp.trim(),type:'phone_change'});if(res.error)throw res.error;phonePending='';status('Mobile verified. Ask admin to issue your card to your account email.');return;}
   let payload={...fields};
